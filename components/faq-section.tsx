...
export function FAQSection() {
  return (
    <section id="faq" className="py-2 px-6 pb-80">
      <div className="max-w-4xl mx-auto">
        <div className="text-center mb-16">
          <h2 className="text-4xl md:text-5xl font-normal mb-6 text-balance font-serif">Câu hỏi thường gặp</h2>
          <p className="text-muted-foreground max-w-2xl mx-auto leading-relaxed">
           Tất cả những gì bạn cần biết về nextgrowth. Có câu hỏi nào không được liệt kê ở đây? Liên hệ với bộ phận hỗ trợ của chúng tôi.
          </p>
        </div>
        ...
        <Accordion type="single" collapsible className="space-y-3 py-0 my-0">
          {faqs.map((faq, index) => (
           <AccordionItem
              key={index}
              value={`item-${index}`}
              className="bg-card border border-border rounded-xl px-6 data-[state=open]:border-foreground/30 overflow-hidden mb-3 border-b last:border-b"
            >
